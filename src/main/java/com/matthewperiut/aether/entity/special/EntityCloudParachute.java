...
public class EntityCloudParachute extends Entity implements EntitySpawnDataProvider {
    ...
    public static void doCloudSmoke(World world, LivingEntity entityliving) {
        double x = entityliving.x + ((EntityAccessor) entityliving).getRandom().nextDouble() * 0.75 * 2.0 - 0.75;
        double y = entityliving.boundingBox.minY - 0.5 + ((EntityAccessor) entityliving).getRandom().nextDouble() * 0.75 * 2.0 - 0.75;
        double z = entityliving.z + ((EntityAccessor) entityliving).getRandom().nextDouble() * 0.75 * 2.0 - 0.75;
    ...
    public static boolean entityHasRoomForCloud(World world, LivingEntity entityliving) {
        ...
        return world.getEntities(entityliving, boundingBox).size() == 0 && !world.isFluidInBox(boundingBox, Material.WATER);
    }
    ...
    private LivingEntity findUser() {
        List entities = this.world.collectEntitiesByClass(LivingEntity.class, this.boundingBox.copy().offset(0.0, 1.0, 0.0));
    ...
    private boolean isCollided() {
        return this.world.getEntities(this, this.boundingBox).size() > 0 || this.world.isFluidInBox(this.boundingBox, Material.WATER);
    }
