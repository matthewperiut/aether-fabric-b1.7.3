...
public class EntityValkyrie extends EntityDungeonMob implements BossLivingEntity, MobSpawnDataProvider {
    ...
    public void tick() {
        ...
        super.tick();
        if (!this.onGround && this.target != null && this.lastMotionY >= 0.0 && this.velocityY < 0.0 && this.getDistance(this.target) <= 16.0F && this.method_928(this.target)) {
        ...
        if (this.isSwinging) {
            // field_1027 > lastHandSwingProgress
            this.field_1027 += 0.15F;
            // field_1035 > handSwingProgress
        ...
        if (!this.otherDimension()) {
            ...
            if (this.timeLeft <= 0) {
                this.dead = true;
                // method_919 > spawned From Spawner
                this.method_919();
            }
        }
    ...
    public void chatLineServer(String s) {
        ...
        for (PlayerEntity player : playersNearby) {
            ((ServerPlayerEntity) player).method_490(s);
        }
    }
    ...
    public void tickLiving() {
        ...
        if (this.teleTimer >= 450) {
            ...
            if (this.teleTimer % 5 == 0 && this.target != null && !this.method_928(this.target)) {
                this.teleTimer += 100;
            }
        } else {
            this.teleTimer = 446;
    ...
    public boolean canSpawn() {
        ...
        return this.world.getLightLevel(i, j, k) > 8 && this.world.canSpawnEntity(this.boundingBox) && this.world.getEntities(this, this.boundingBox).size() == 0 && !this.world.isBoxSubmergedInFluid(this.boundingBox);
    }
        ...
        if (this.boss) {
            for (int k = this.dungeonZ + 2; k < this.dungeonZ + 23; k += 7) {
                if (this.world.getBlockId(this.dungeonX - 1, this.dungeonY, k) == 0) {
                    ...
                    this.world.setBlock(this.dungeonX - 1, this.dungeonY, k, AetherBlocks.LockedDungeonStone.id, 1);
                    this.world.setBlock(this.dungeonX - 1, this.dungeonY, k + 1, AetherBlocks.LockedDungeonStone.id, 1);
                    this.world.setBlock(this.dungeonX - 1, this.dungeonY + 1, k + 1, AetherBlocks.LockedDungeonStone.id, 1);
                    this.world.setBlock(this.dungeonX - 1, this.dungeonY + 1, k, AetherBlocks.LockedDungeonStone.id, 1);
                    return;
                }
            }
        }
    ...
    private void unlockTreasure() {
        this.world.setBlock(this.dungeonX + 16, this.dungeonY + 1, this.dungeonZ + 9, Block.TRAPDOOR.id, 3);
        this.world.setBlock(this.dungeonX + 17, this.dungeonY + 1, this.dungeonZ + 9, Block.TRAPDOOR.id, 2);
        this.world.setBlock(this.dungeonX + 16, this.dungeonY + 1, this.dungeonZ + 10, Block.TRAPDOOR.id, 3);
        this.world.setBlock(this.dungeonX + 17, this.dungeonY + 1, this.dungeonZ + 10, Block.TRAPDOOR.id, 2);
            ...
            for (int y = this.dungeonY - 1; y < this.dungeonY + 22; ++y) {
                for (int z = this.dungeonZ - 5; z < this.dungeonZ + 25; ++z) {
                    ...
                    if (id == AetherBlocks.LockedDungeonStone.id) {
                        this.world.setBlock(x, y, z, AetherBlocks.DungeonStone.id, this.world.getBlockMeta(x, y, z));
                    }
                    ...
                    if (id == AetherBlocks.LockedLightDungeonStone.id) {
                        this.world.setBlock(x, y, z, AetherBlocks.LightDungeonStone.id, this.world.getBlockMeta(x, y, z));
                    }
                }
            }
        }
