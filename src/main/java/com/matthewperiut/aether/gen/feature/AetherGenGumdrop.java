...
public class AetherGenGumdrop extends Feature
{
    ...
    public boolean func_100009_a(World world, Random random, int i, int j, int k, int l)
    {
        ...
        for (i2 = -l; i2 <= l; ++i2)
        {
            for (l2 = l; l2 >= -l; --l2)
            {
                for (i3 = -l; i3 <= l; ++i3)
                {
                    ...
                    if (Math.sqrt((double) (k3 * k3 + i4 * i4 + k4 * k4)) <= (double) l)
                    {
        ...
        for (i3 = 0; i3 < i2; ++i3)
        {
            ...
            k4 = i + MathHelper.floor(Math.cos((double) (f1 * f2)) * (double) f3);
            j5 = j - MathHelper.floor((double) l * (double) random.nextFloat() * 0.3);
            j6 = k + MathHelper.floor(-Math.sin((double) (f1 * f2)) * (double) f3);
        ...
        for (i1 = -l; i1 <= l; ++i1)
        {
            for (k1 = l; k1 >= -l; --k1)
            {
                for (i2 = -l; i2 <= l; ++i2)
                {
                    ...
                    if (Math.sqrt((double) (k2 * k2 + i3 * i3 + k3 * k3)) <= (double) l)
                    {
