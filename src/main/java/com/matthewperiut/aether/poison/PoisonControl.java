...
public class PoisonControl {
    ...
    public void distractEntity() {
        double gauss = ((EntityAccessor) parent).getRandom().nextGaussian();
    ...
    private static void informClientMovement(PlayerEntity player) {
        if (player instanceof ServerPlayerEntity serverPlayer) {
            serverPlayer.field_255.handle(new EntityVelocityUpdateS2CPacket(serverPlayer.id, serverPlayer.velocityX, serverPlayer.velocityY, serverPlayer.velocityZ));
        }
    }
}
