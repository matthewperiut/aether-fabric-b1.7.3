    ...
    public void render(Entity entity, double d, double d1, double d2, float f, float f1) {
        ...
        if (!entityWhirl.evil) {
            ...
            for (i = 0; i < 3; ++i) {
                ...
                d8 = (double) ((float) entityWhirl.z + rand.nextFloat() * 0.25F);
                f2 = rand.nextFloat() * 360.0F;
                SmokeParticleEntity entitySmokeFx = new SmokeParticleEntity(entityWhirl.world, -Math.sin((double) (0.01745329F * f2)) * 0.75, d7 - 0.25, Math.cos((double) (0.01745329F * f2)) * 0.75, d6, 0.125, d8, 3.5F);
        ...
        if (entityWhirl.fluffies.size() > 0) {
            for (i = 0; i < entityWhirl.fluffies.size(); ++i) {
                ParticleEntity entityFx = (ParticleEntity) entityWhirl.fluffies.get(i);
                if (entityFx.removed) {
                    ...
                    double d9 = (double) entityWhirl.distanceTo(entityFx);
                    double d10 = d7 - d4;
                    entityFx.yVelocity = 0.11500000208616257;
                    double d11 = Math.atan2(d3 - d6, d5 - d8) / 0.01745329424738884;
                    d11 += 160.0;
                    entityFx.xVelocity = -Math.cos(0.01745329424738884 * d11) * (d9 * 2.5 - d10) * 0.10000000149011612;
                    entityFx.zVelocity = Math.sin(0.01745329424738884 * d11) * (d9 * 2.5 - d10) * 0.10000000149011612;
                }
            }
        }
    }
