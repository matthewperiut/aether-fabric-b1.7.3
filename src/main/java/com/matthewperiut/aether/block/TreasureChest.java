...
public class TreasureChest extends TemplateChestBlock {
    ...
    public boolean onUse(final World level, final int x, final int y, final int z, final PlayerEntity player) {
        ...
        BlockEntity blockentity = level.getBlockEntity(x, y, z);
        if (blockentity instanceof BlockEntityTreasureChest treasureChest) {
            if (meta == 5) {
            ...
            if (meta > 1) {
                ((BlockEntityTreasureChest) blockentity).setRarity(meta - 1);
                    ...
                    switch (treasureChest.rarity) {
                        ...
                        case 1:
                            for (int p = 0; p < 3 + random.nextInt(3); ++p) {
                                ...
                                treasureChest.setStack(random.nextInt(treasureChest.size()), item);
                            }
                            break;
                        case 2:
                            for (int p = 0; p < 3 + random.nextInt(3); ++p) {
                                final ItemStack item = this.getSilverLoot(random);
                                treasureChest.setStack(random.nextInt(treasureChest.size()), item);
                            }
                            break;
                        case 3:
